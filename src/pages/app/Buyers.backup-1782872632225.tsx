...
export default function Buyers() {
  ...
  const persistBuyerUpdate = async (id: string, updates: any, successMessage?: string) => {
    ...
    const result = await updateBuyerInSupabase(cleanId, {
      ...updates,
      id: cleanId,
    ...
    updateBuyer(cleanId, (result.data || { ...updates, id: cleanId }) as any)
    if (selectedBuyer?.id === cleanId) {
      setSelectedBuyer((result.data || { ...selectedBuyer, ...updates, id: cleanId }) as any)
      setEditBuyer((result.data || { ...editBuyer, ...updates, id: cleanId }) as any)
    }
...
const [showImport, setShowImport] = useState(false)
  ...
  function hydrateBuyerForEdit(buyer: any) {
    ...
    return {
      ...
      budgetMin: asMoney(merged.budgetMin, merged.budget_min, merged.minBudget, merged.min_budget, merged.priceMin, merged.price_min),
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-sm">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  <div><div className="text-xs text-[#8B92A3]">Buyer Name</div><input className="input" value={editBuyer.name || ''} onChange={e => setEditBuyer({...editBuyer, name: e.target.value})} placeholder={getDisplayName(selectedBuyer)} /></div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1 text-[10px] border border-[#252A38] rounded-xl p-2 bg-[#070A0F]">
                      {BUYER_STRATEGY_OPTIONS.map(strategy => {
                        const current = Array.isArray(editBuyer.strategies)
                    ...
                    {(!Array.isArray(editBuyer.strategies) || editBuyer.strategies.length === 0) && (
                ...
                {addBuyerMode === 'detailed' && (
                  <>
                    <div>
                      ...
                      <input className="input" type="number" value={manualBuyer.budgetMin} onChange={e => setManualBuyer({ ...manualBuyer, budgetMin: e.target.value })} />
                    </div>
                    <div>
                          ...
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              <label>
                                ...
                                <input className="input" value={row.budgetMin || ''} onChange={e => updatePortalPendingRow(row._id, { budgetMin: Number(e.target.value) || 0 })} />
                              </label>
                              <label>
