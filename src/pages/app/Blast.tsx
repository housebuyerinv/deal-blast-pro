...
export default function Blast() {
  const { 
    deals: storeDeals, getMatchesForDeal, recordBlast, logActivity, 
    recordBlastLog, getBlastLogs, dealSuppressions, scheduleFollowUp 
  } = useAppStore()

  const blastableDeals = useMemo(() => {
    const seen = new Set<string>()

    return (storeDeals || []).filter((deal: any) => {
      if (!deal?.id || seen.has(deal.id)) return false
      if (!BLASTABLE_STATUSES.has(String(deal.status || ''))) return false
      seen.add(deal.id)
      return true
    }).map((deal: any) => ({
      ...deal,
      ...
      docs: Array.isArray(deal.docs) ? deal.docs : [],
    }))
  }, [storeDeals])
  const deals = blastableDeals

  const [dealId, setDealId] = useState(blastableDeals[0]?.id || '')
  ...
  const [recipientLimit, setRecipientLimit] = useState<number>(100)

  const selectedDeal = blastableDeals.find((d: any) => d.id === dealId)
  const deal = selectedDeal ? {
    ...selectedDeal,
    property: selectedDeal.property || {},
    pricing: selectedDeal.pricing || {},
    submitter: selectedDeal.submitter || {},
    docs: Array.isArray(selectedDeal.docs) ? selectedDeal.docs : [],
  } : undefined
  const allMatches = deal ? getMatchesForDeal(dealId) : []

  useEffect(() => {
    if (dealId && !blastableDeals.some((d: any) => d.id === dealId)) {
      setDealId(blastableDeals[0]?.id || '')
      setSelectedBuyers([])
      setSelectedDocIds([])
    } else if (!dealId && blastableDeals.length) {
      setDealId(blastableDeals[0].id)
    }
  }, [dealId, blastableDeals])

  const previewProperty = deal?.property || {}

  // Live preview tokens
  const previewSubject = useMemo(() => {
    if (!deal) return ''
    return (subject || TEMPLATES[templateKey]?.subject || '')
      .replace(/{{city}}/g, previewProperty.city || '')
      .replace(/{{state}}/g, previewProperty.state || '')
      .replace(/{{address}}/g, previewProperty.address || '')
      .replace(/{{type}}/g, previewProperty.type || 'Deal')
  ...
  const currentDealSuppressions = deal ? (dealSuppressions[dealId] || []) : []
  const filteredMatches = allMatches.filter(m => 
    !suppressionList.includes(safeLower(m.buyer.email)) &&
  ...
  const displayedMatches = recipientLimit === -1 ? sortedMatches : sortedMatches.slice(0, recipientLimit);

  const cleanMatchText = (value: any) => String(value || '')
    .replace(/\(\+\d+\)/g, '')
    .replace(/\s+/g, ' ')
    .trim();

  const buyerMarketLabel = (buyer: any) => {
    const list = Array.isArray(buyer?.markets) ? buyer.markets.filter(Boolean) : [];
    return list.length ? list.join(', ') : 'Markets not set';
  };

  const buyerAssetLabel = (buyer: any) => {
    const list = Array.isArray(buyer?.assetTypes) ? buyer.assetTypes.filter(Boolean) : [];
    return list.length ? list.join(', ') : 'Assets not set';
  };
  ...
  const sendBlast = () => {
    ...
    if (!deal || !dealId || toSend.length === 0) {
    ...
    <div className="max-w-6xl">
      ...
      <div className="grid lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <button onClick={sendBlast} disabled={!deal} className="btn btn-green w-full py-3 text-base">SEND BLAST + LOG ACTIVITY</button>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {deal && getBlastLogs(dealId).length > 0 && (
            <div className="card p-4">
          ...
          <div className="card p-4">
            ...
            <div className="bg-[#0A0C12] p-4 rounded border border-[#252A38] text-sm">
              {!deal ? (
                <div className="text-[#8B92A3] text-xs">Select a deal to preview blast copy.</div>
              ) : (
                <>
          ...
          <div className="card p-4">
            ...
            <div className="mb-3">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                <button onClick={() => setRecipientLimit(100)} className="btn btn-ghost text-xs px-2 py-0.5">Display 100</button>
                <button onClick={() => setRecipientLimit(250)} className="btn btn-ghost text-xs px-2 py-0.5">Display 250</button>
                <button onClick={() => setRecipientLimit(-1)} className="btn btn-ghost text-xs px-2 py-0.5">Display All</button>
                ...
                <div className="flex gap-1 text-xs">
                  ...
                  <button onClick={() => setSelectedBuyers(filteredMatches.filter(m => !suppressionList.includes(safeLower(m.buyer.email))).map(m=>m.buyer.id).slice(0,50))} className="btn btn-ghost px-1.5 py-0.5">Unsuppressed</button>
                </div>
              </div>

              <div className="text-xs text-[#8B92A3] flex gap-3">
                <span>Visible: {filteredMatches.length}</span>
                <span>Showing: {displayedMatches.length} of {sortedMatches.length}</span>
            ...
            <div className="max-h-[70vh] min-h-[520px] overflow-auto space-y-2 text-sm pr-2">
              {displayedMatches.map((m: any) => {
                const positive = (m.reasons || []).map(cleanMatchText).filter(Boolean).slice(0, 6);
                const negatives = (m.missing || []).map(cleanMatchText).filter(Boolean).slice(0, 4);
                return (
                  <label key={m.buyer.id} className="flex items-start gap-3 panel p-3 cursor-pointer">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[10px] mt-1 text-[#8B92A3] break-words">{m.buyer.email} • {m.buyer.budgetMax ? '$' + Math.round(m.buyer.budgetMax/1000) + 'K max' : 'Budget n/a'}</div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-1 mt-2 text-[10px] text-[#C5CAD6]">
                        <div className="rounded bg-[#0A0C12] border border-[#252A38] px-2 py-1">
                          <span className="text-[#8B92A3]">Markets:</span> {buyerMarketLabel(m.buyer)}
                        </div>
                        <div className="rounded bg-[#0A0C12] border border-[#252A38] px-2 py-1">
                          <span className="text-[#8B92A3]">Assets:</span> {buyerAssetLabel(m.buyer)}
                        </div>
                      </div>

                      {/* Minimal qualification reasoning */}
                      <div className="mt-2 text-[10px] flex flex-wrap gap-1">
                        {positive.map((r: string, i: number) => <span key={i} className="rounded border border-emerald-500/30 bg-emerald-500/10 text-emerald-300 px-2 py-0.5">Match: {r.split('(')[0].trim()}</span>)}
                        {negatives.map((r: string, i: number) => <span key={i} className="rounded border border-amber-500/30 bg-amber-500/10 text-amber-300 px-2 py-0.5">Needs: {r}</span>)}
                      </div>
                    </div>
                    ...
                    {selectedBuyers.includes(m.buyer.id) && (
                      <div className="flex gap-1 text-[10px] whitespace-nowrap">
                        <button onClick={(e) => { e.preventDefault(); window.location.href = `/app/buyers?search=${encodeURIComponent(m.buyer.email || m.buyer.name || m.buyer.id)}`; }} className="text-[#3B82F6] hover:underline">Find Buyer</button>
