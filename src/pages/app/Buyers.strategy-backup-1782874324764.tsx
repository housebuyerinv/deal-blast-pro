...
export default function Buyers() {
  ...
  const persistBuyerUpdate = async (id: string, updates: any, successMessage?: string) => {
    ...
    const cleanUpdates = {
      ...updates,
      budgetMin: normalizeBuyerBudgetMin(updates?.budgetMin),
      budgetMax: normalizeBuyerBudgetMax(updates?.budgetMax),
      proofFiles: getBuyerProofFiles(updates).length ? getBuyerProofFiles(updates) : updates?.proofFiles,
      uploadedFiles: Array.isArray(updates?.uploadedFiles) ? updates.uploadedFiles : updates?.uploadedFiles,
      id: cleanId,
    ...
    updateBuyer(cleanId, (result.data || cleanUpdates) as any)
    if (selectedBuyer?.id === cleanId) {
      setSelectedBuyer((result.data || { ...selectedBuyer, ...cleanUpdates }) as any)
      setEditBuyer((result.data || { ...editBuyer, ...cleanUpdates }) as any)
    }
