...
export async function handlePlatformAction(action:string,req:any,res:any,account:any){
  ...
  if(action==='admin-customer-lifecycle'){
    ...
    const [workspacesResult,profilesResult,plansResult,buyersResult,dealsResult]=await Promise.all([
      account.adminClient.from('workspaces').select('id,name,owner_user_id,owner_email,created_at,account_status').order('created_at',{ascending:false}).limit(2000),
      account.adminClient.from('account_profiles').select('user_id,email,full_name,display_name,account_status,created_at,dismissed_promotion_key,product_updates_opt_in').limit(2000),
      account.adminClient.from('workspace_plan_assignments').select('workspace_id,user_id,plan_name,current_plan,effective_access_plan,billing_status,payment_status,subscription_status,trial_status,trial_started_at,trial_ends_at,trial_converted_at,trial_consumed_at,first_paid_at,stripe_customer_id,stripe_subscription_id,property_intelligence_included_credits,created_at').limit(2000),
      account.adminClient.from('workspace_buyers').select('workspace_id').limit(10000),
      account.adminClient.from('inventory_deals').select('workspace_id').limit(10000),
    ])
