-- Forward-only repair for converted submissions that could not be backfilled
-- until submission workspace assignment became durable.
--
-- Duplicate safety is enforced both by NOT EXISTS and the inventory table's
-- unique (workspace_id, source_submission_id) constraint. Original submission
-- identifiers, conversion timestamps, and JSON audit history are preserved.
